...
public class Spec2Fixture : SpecFixture
{
    ...
    public static void CompileAndRun(IRenderConfig renderConfig, string outputDir, Action<SmRunner>? action = null, string semiColon = ";", string trueString = "true", TranspilerId transpilerId = TranspilerId.Default, IServiceProvider? serviceProvider = null)
    {
        var diagramFile = Spec2Directory + "Spec2Sm.graphml";
        CompileAndRun(renderConfig, diagramFile, outputDir, smRunnerAction: action, semiColon: semiColon, trueString: trueString, transpilerId: transpilerId, serviceProvider: serviceProvider);
    }
}
