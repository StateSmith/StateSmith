// Autogenerated with StateSmith.
// Algorithm: Balanced2. See https://github.com/StateSmith/StateSmith/wiki/Algorithms

export enum EventId
{
    EV1 = 0,
    EV2 = 1,
}

export const EventIdCount: number = 2;

export enum StateId
{
    ROOT = 0,
    GROUP = 1,
    G1 = 2,
    G2 = 3,
    S1 = 4,
}

export const StateIdCount: number = 5;

// Subtree meta data generation can be disabled in settings.
// Details: https://github.com/StateSmith/StateSmith/issues/538
export const ROOT_SubtreeEndId: number = 4;  // State 'RocketSm' subtree extends from itself (id: 0) to state 's1' (id: 4)
export const GROUP_SubtreeEndId: number = 3;  // State 'group' subtree extends from itself (id: 1) to state 'g2' (id: 3)
export const G1_SubtreeEndId: number = 2;  // State 'g1' subtree extends from itself (id: 2) to state 'g1' (id: 2)
export const G2_SubtreeEndId: number = 3;  // State 'g2' subtree extends from itself (id: 3) to state 'g2' (id: 3)
export const S1_SubtreeEndId: number = 4;  // State 's1' subtree extends from itself (id: 4) to state 's1' (id: 4)

// Generated state machine
export class RocketSm extends RocketSmBase implements IRocketSm
{
    public userCodeMethod() {
        console.log('userCodeMethod');
    }
    
    
    // Used internally by state machine. Feel free to inspect, but don't modify.
    public stateId: StateId;
    
    // State machine constructor. Must be called before start or dispatch event functions. Not thread safe.
    public RocketSm()
    {
    }
    
    // Starts the state machine. Must be called before dispatching events. Not thread safe.
    public start(): void 
    {
        this.ROOT_enter();
        // ROOT behavior
        // uml: TransitionTo(ROOT.<InitialState>)
        {
            // Step 1: Exit states until we reach `ROOT` state (Least Common Ancestor for transition). Already at LCA, no exiting required.
            
            // Step 2: Transition action: ``.
            
            // Step 3: Enter/move towards transition target `ROOT.<InitialState>`.
            // ROOT.<InitialState> is a pseudo state and cannot have an `enter` trigger.
            
            // ROOT.<InitialState> behavior
            // uml: TransitionTo(group)
            {
                // Step 1: Exit states until we reach `ROOT` state (Least Common Ancestor for transition). Already at LCA, no exiting required.
                
                // Step 2: Transition action: ``.
                
                // Step 3: Enter/move towards transition target `group`.
                this.GROUP_enter();
                
                // group.<InitialState> behavior
                // uml: TransitionTo(g1)
                {
                    // Step 1: Exit states until we reach `group` state (Least Common Ancestor for transition). Already at LCA, no exiting required.
                    
                    // Step 2: Transition action: ``.
                    
                    // Step 3: Enter/move towards transition target `g1`.
                    this.G1_enter();
                    
                    // Step 4: complete transition. Ends event dispatch. No other behaviors are checked.
                    return;
                } // end of behavior for group.<InitialState>
            } // end of behavior for ROOT.<InitialState>
        } // end of behavior for ROOT
    }
    
    // Dispatches an event to the state machine. Not thread safe.
    // Note! This function assumes that the `eventId` parameter is valid.
    public dispatchEvent(eventId: EventId): void 
    {
        switch (this.stateId)
        {
            // STATE: RocketSm
            case StateId.ROOT:
                // No events handled by this state (or its ancestors).
                break;
            
            // STATE: group
            case StateId.GROUP:
                switch (eventId)
                {
                    case EventId.EV1: this.GROUP_ev1(); break;
                }
                break;
            
            // STATE: g1
            case StateId.G1:
                switch (eventId)
                {
                    case EventId.EV1: this.G1_ev1(); break;
                }
                break;
            
            // STATE: g2
            case StateId.G2:
                switch (eventId)
                {
                    case EventId.EV2: this.G2_ev2(); break;
                    case EventId.EV1: this.GROUP_ev1(); break; // First ancestor handler for this event
                }
                break;
            
            // STATE: s1
            case StateId.S1:
                // No events handled by this state (or its ancestors).
                break;
        }
        
    }
    
    // This function is used when StateSmith doesn't know what the active leaf state is at
    // compile time due to sub states or when multiple states need to be exited.
    private exitUpToStateHandler(desiredState: StateId): void 
    {
        while (this.stateId != desiredState)
        {
            switch (this.stateId)
            {
                case StateId.GROUP: this.GROUP_exit(); break;
                
                case StateId.G1: this.G1_exit(); break;
                
                case StateId.G2: this.G2_exit(); break;
                
                case StateId.S1: this.S1_exit(); break;
                
                default: return;  // Just to be safe. Prevents infinite loop if state ID memory is somehow corrupted.
            }
        }
    }
    
    
    ////////////////////////////////////////////////////////////////////////////////
    // event handlers for state ROOT
    ////////////////////////////////////////////////////////////////////////////////
    
    private ROOT_enter(): void 
    {
        this.stateId = StateId.ROOT;
    }
    
    
    ////////////////////////////////////////////////////////////////////////////////
    // event handlers for state GROUP
    ////////////////////////////////////////////////////////////////////////////////
    
    private GROUP_enter(): void 
    {
        this.stateId = StateId.GROUP;
    }
    
    private GROUP_exit(): void 
    {
        this.stateId = StateId.ROOT;
    }
    
    private GROUP_ev1(): void 
    {
        // group behavior
        // uml: EV1 TransitionTo(s1)
        {
            // Step 1: Exit states until we reach `ROOT` state (Least Common Ancestor for transition).
            this.exitUpToStateHandler(StateId.ROOT);
            
            // Step 2: Transition action: ``.
            
            // Step 3: Enter/move towards transition target `s1`.
            this.S1_enter();
            
            // Step 4: complete transition. Ends event dispatch. No other behaviors are checked.
            return;
        } // end of behavior for group
        
        // No ancestor handles this event.
    }
    
    
    ////////////////////////////////////////////////////////////////////////////////
    // event handlers for state G1
    ////////////////////////////////////////////////////////////////////////////////
    
    private G1_enter(): void 
    {
        this.stateId = StateId.G1;
    }
    
    private G1_exit(): void 
    {
        this.stateId = StateId.GROUP;
    }
    
    private G1_ev1(): void 
    {
        let consume_event: boolean = false;
        
        // g1 behavior
        // uml: EV1 [a > 20] TransitionTo(g2)
        if (a > 20)
        {
            // Step 1: Exit states until we reach `group` state (Least Common Ancestor for transition).
            this.G1_exit();
            
            // Step 2: Transition action: ``.
            
            // Step 3: Enter/move towards transition target `g2`.
            this.G2_enter();
            
            // Step 4: complete transition. Ends event dispatch. No other behaviors are checked.
            return;
        } // end of behavior for g1
        
        // Check if event has been consumed before calling ancestor handler.
        if (!consume_event)
        {
            this.GROUP_ev1();
        }
    }
    
    
    ////////////////////////////////////////////////////////////////////////////////
    // event handlers for state G2
    ////////////////////////////////////////////////////////////////////////////////
    
    private G2_enter(): void 
    {
        this.stateId = StateId.G2;
    }
    
    private G2_exit(): void 
    {
        this.stateId = StateId.GROUP;
    }
    
    private G2_ev2(): void 
    {
        // g2 behavior
        // uml: EV2 TransitionTo(g1)
        {
            // Step 1: Exit states until we reach `group` state (Least Common Ancestor for transition).
            this.G2_exit();
            
            // Step 2: Transition action: ``.
            
            // Step 3: Enter/move towards transition target `g1`.
            this.G1_enter();
            
            // Step 4: complete transition. Ends event dispatch. No other behaviors are checked.
            return;
        } // end of behavior for g2
        
        // No ancestor handles this event.
    }
    
    
    ////////////////////////////////////////////////////////////////////////////////
    // event handlers for state S1
    ////////////////////////////////////////////////////////////////////////////////
    
    private S1_enter(): void 
    {
        this.stateId = StateId.S1;
    }
    
    private S1_exit(): void 
    {
        this.stateId = StateId.ROOT;
    }
    
    // Thread safe. This function can be disabled with `outputStateIdToStringFunction` setting.
    public static stateIdToString(id: StateId): string 
    {
        switch (id)
        {
            case StateId.ROOT: return "ROOT";
            case StateId.GROUP: return "GROUP";
            case StateId.G1: return "G1";
            case StateId.G2: return "G2";
            case StateId.S1: return "S1";
            default: return "?";
        }
    }
    
    // Thread safe. This function can be disabled with `outputEventIdToStringFunction` setting.
    public static eventIdToString(id: EventId): string 
    {
        switch (id)
        {
            case EventId.EV1: return "EV1";
            case EventId.EV2: return "EV2";
            default: return "?";
        }
    }
    
    // Returns the parent state for a given state. Returns ROOT if input has no parent.
    // Thread safe. This function can be disabled with `outputGetParentIdFunction` setting.
    public static getParentId(id: StateId): StateId 
    {
        switch (id)
        {
            case StateId.ROOT: return StateId.ROOT;
            case StateId.GROUP: return StateId.ROOT;
            case StateId.G1: return StateId.GROUP;
            case StateId.G2: return StateId.GROUP;
            case StateId.S1: return StateId.ROOT;
            default: return StateId.ROOT;
        }
    }
}
