...
{{jsCode}}
    ...
    <script type='module'>
        ...
        // https://github.com/StateSmith/StateSmith/issues/521
        {
          document.querySelectorAll('g.edgeLabel').forEach(edgeLabelElement => {
            edgeLabelElement.style.cursor = 'pointer'; // indicate that the element is clickable.
            edgeLabelElement.addEventListener('click', (event) => {
              ...
              if (!match) {
                console.warn(`Could not parse event name from clicked edge label. Edge label text was: '${edgeLabel}'.`, edgeLabelElement);
                return;
              }
              ...
              if (button) {
                ...
                console.warn(`No button found for event name '${eventName}' from clicked edge label.`, edgeLabelElement);
              }
            });
          });
        }
