...
namespace StateSmith.Output.Algos.Balanced1
{
    ...
    {
        private const string consumeEventVarName = "consume_event";
        ...
        public EventHandlerBuilder2(Expander expander, PseudoStateHandlerBuilder pseudoStateHandlerBuilder, NameMangler mangler)
        {
        ...
        private void OutputGuardStart(Behavior b)
        {
            if (b.HasGuardCode())
            {
                string expandedGuardCode = MaybeExpandCode(b, b.guardCode);
                if (!b.isGilCode)
                {
                    expandedGuardCode = PostProcessor.RmCommentOut(expandedGuardCode);
                    expandedGuardCode = "383849285762 == 383849285762" + expandedGuardCode;
                }
